<template>
  <div :class="['app-card', `app-card--${variant}`]">
    <slot />
  </div>
</template>
...
withDefaults(defineProps<{
  variant?: 'default' | 'bordered' | 'elevated' | 'glass' | 'metric'
}>(), {
  variant: 'default',
})
</script>
...
.app-card--metric {
  background: linear-gradient(180deg, rgba(59, 89, 152, 0.06), var(--color-surface));
  border: 1px solid rgba(26, 59, 102, 0.08);
  box-shadow: var(--shadow-card);
}

.app-card--metric:hover {
  box-shadow: var(--shadow-md);
}
